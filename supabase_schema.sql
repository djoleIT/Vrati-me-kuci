...
-- Dodatak: tip pričvršćivanja (traka za navlačenje / privezak sa alkicom)
-- i da li se ime ljubimca fizički štampa na privesku — bezbedno za ponovno pokretanje
alter table orders add column if not exists attachment_type text default 'ring';
alter table orders add column if not exists show_name_on_tag boolean default true;
