...
## NOVO — statistika skeniranja i nalog za vlasnike
1. Pokreni ponovo ceo `supabase_schema.sql` u Supabase SQL Editoru (dodaje `scan_events` tabelu — bezbedno za ponovno pokretanje)
2. Vlasnici sad mogu da odu na `/moj-profil`, unesu isti email koji su ostavili prilikom narudžbine,
   dobiju magic-link email (bez lozinke), i vide koliko puta je njihov QR skeniran + menjaju svoje podatke
3. Magic-link email šalje Supabase automatski (ugrađeno, ne treba poseban email servis) — besplatni
   tier ima ograničenje broja email-ova na sat, dovoljno za početak. Za veći obim, u Supabase →
   Authentication → Email podešavanjima možeš da povežeš sopstveni SMTP kasnije
4. Admin panel sad ima kolonu "Skeniranja" — broj i vreme poslednjeg skeniranja po narudžbini
